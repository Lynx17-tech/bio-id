...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Update Logo in Header based on Municipality
    const headerLogo = document.getElementById('header-muni-logo');
    const printLogos = document.querySelectorAll('.print-muni-logo');
    
    const logoErrorHandler = function () {
    ...
    if (headerLogo) {
        ...
        headerLogo.onerror = logoErrorHandler;
    }
    
    printLogos.forEach(logo => {
        logo.onerror = logoErrorHandler;
    });

    if (activeMunicipality) {
        const logoName = activeMunicipality.toLowerCase().replace(/\s+/g, '_');
        const logoUrl = `../images/${logoName}_logo.png`;
        if (headerLogo) headerLogo.src = logoUrl;
        printLogos.forEach(logo => logo.src = logoUrl);
    }
